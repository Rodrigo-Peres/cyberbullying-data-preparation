...
class DatasetCleaner(ABC):
    def __cal__(self, text: str | list[str]) -> None:
...
class StopWordsDatasetClear(DatasetCleaner):
    ...
    def clean_words(self, words: list[str]) -> list[str]:
        cleaned_text = [word for word in words if word not in self.stopwords]
        return "".join(cleaned_text)
...
class URLDatasetCleaner(DatasetCleaner):
    def clean_text(self, text: str) -> str:
        return re.sub(r"http\+", "", text, flags=re.MULTILINE)
