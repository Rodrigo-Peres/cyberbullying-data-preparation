...
class DatasetReader(ABC):
    ...
    def split_dataset(self, df: dd.core.DataFrame, test_size: float, stratify_column: Optional[str] = None) -> tuple[dd.core.DataFrame, dd.core.DataFrame]:
        if stratify_column is None:
            return train_test_split(df, test_size=test_size, shuffle=True, random_state=1234)
...
class TwitterDatasetReader(DatasetReader):
    ...
    def get_text_and_label_col(self, df: dd.core.DataFrame) -> dd.core.DataFrame:
        df['label'] = df['cyberbullying_type'].apply(lambda x: 1 if x != "not_cyberbullying" else 0)
